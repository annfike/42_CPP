...
class Animal {
    ...
    public:
		...
		virtual void    makeSound() const; // Ключевое слово virtual позволяет переопределять этот метод в наследуемых классах. Также const указывает, что метод не изменяет состояние объекта.
